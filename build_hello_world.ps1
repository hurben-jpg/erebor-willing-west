...
Write-Host "Building Hello World..."
idf.py build

if (Test-Path "build\hello_world.elf") {
    Write-Host "Build successful! Regenerating binary with correct chip revision..."
    & "C:\Users\hurben\.espressif\python_env\idf5.3_py3.11_env\Scripts\python.exe" "C:/Espressif/frameworks/esp-idf-v5.3.1/components/esptool_py/esptool/esptool.py" --chip esp32p4 elf2image --min-rev-full 100 --flash_mode dio --flash_freq 80m --flash_size 2MB -o "build\hello_world_fixed.bin" "build\hello_world.elf"

    Write-Host "Flashing to COM4..."
    & "C:\Users\hurben\.espressif\python_env\idf5.3_py3.11_env\Scripts\python.exe" "C:/Espressif/frameworks/esp-idf-v5.3.1/components/esptool_py/esptool/esptool.py" --chip esp32p4 -p COM4 -b 460800 --before=default_reset --after=hard_reset write_flash --flash_mode dio --flash_freq 80m --flash_size 2MB 0x2000 "build\bootloader\bootloader.bin" 0x8000 "build\partition_table\partition-table.bin" 0x10000 "build\hello_world_fixed.bin"
}
else {
    Write-Host "Build failed - ELF file not found"
    Write-Host "Trying to set target first..."
    idf.py set-target esp32p4
    idf.py build
    
    if (Test-Path "build\hello_world.elf") {
        Write-Host "Build successful after set-target! Regenerating binary..."
        & "C:\Users\hurben\.espressif\python_env\idf5.3_py3.11_env\Scripts\python.exe" "C:/Espressif/frameworks/esp-idf-v5.3.1/components/esptool_py/esptool/esptool.py" --chip esp32p4 elf2image --min-rev-full 100 --flash_mode dio --flash_freq 80m --flash_size 2MB -o "build\hello_world_fixed.bin" "build\hello_world.elf"
        ...
        Write-Host "Build failed even after set-target"
    }
}
