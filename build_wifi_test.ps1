...
cd 'd:\PROJECTS\Antigravity\Erebor\ESP32-P4-WIFI6-Touch-LCD-XC-Demo\ESP-IDF\04_wifistation'

# Force remove sdkconfig to ensure defaults are picked up
if (Test-Path sdkconfig) { Remove-Item sdkconfig }
if (Test-Path sdkconfig.old) { Remove-Item sdkconfig.old }
if (Test-Path build) { Remove-Item build -Recurse -Force }

Write-Host "Setting target to ESP32-P4..."
idf.py set-target esp32p4

Write-Host "Building WiFi Station example..."
idf.py build
